...
static struct capn_segment *create(void *u, uint32_t id, int sz)
{
	...
	s = (struct capn_segment*)kcalloc(1, sz, GFP_KERNEL);
	s->data = (char*)(s + 1);
	s->cap = sz - sizeof(*s);
	s->user = s;
	return s;
...
static int read_fp(void *p, size_t sz, struct capn_stream *z, uint8_t* zbuf, int packed)
{
	if(packed) {
...
static int init_fp(struct capn *c, struct capn_stream *z, int packed)
{
	/*
	...
	uint32_t *hdr = kvcalloc(1024, sizeof(uint32_t), GFP_KERNEL);
	uint8_t *zbuf = kvcalloc(ZBUF_SZ, sizeof(uint8_t), GFP_KERNEL);
	char *data = NULL;

	capn_init_malloc(c);

	/* Read the first four bytes to know how many headers we have */
	if(read_fp(&segnum, 4, z, zbuf, packed))
		goto err;
	...
	if(read_fp(hdr, 8 * (segnum / 2) + 4, z, zbuf, packed))
		goto err;

	for(i = 0; i < segnum; i++) {
		uint32_t n = capn_flip32(hdr[i]);
		if(n > INT_MAX / 8 || n > U32_MAX / 8 || U32_MAX - total < n * 8)
			goto err;
		hdr[i] = n * 8;
		total += hdr[i];
	}
	...
	s = (struct capn_segment*)kcalloc(1, total + (sizeof(*s) * segnum),	GFP_KERNEL);
	if(!s)
		goto err;

	/* Now read the data and setup the capn_segment structs */
	data = (char*)(s + segnum);
	if(read_fp(data, total, z, zbuf, packed))
		goto err;
...
int capn_init_fp(struct capn *c, int packed)
{
	struct capn_stream z;
	memset(&z, 0, sizeof(z));
	return init_fp(c, &z, packed);
}
